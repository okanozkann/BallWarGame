...
function addEnemy(){
    for(var i = enemies.length;i<maxenemiey;i++){
        ...
        var s =0.5+((40 - ((r/40)*r))/160)/maxenemiey;
        ...
        enemies.push(new Circle(x,y,player.x,player.y,r,color,s))
    }
}
...
function animate(){
    ...
    enemies.forEach((enemy,e)=>{
        ...
        if(collision(enemy.x,enemy.y,enemy.r,bullet.x,bullet.y,bullet.r)){
            ...
            score += 25;
            kills ++;
                if(kills>0 && kills%5 === 0){maxenemiey++;}
            addEnemy();
        }else{
                enemy.r-=5;
                score +=5;
        ...
        if(collision(enemy.x,enemy.y,enemy.r,player.x,player.y,player.r)){
            ...
            p.innerHTML="Game Over <br/> Score : "+score+"<br/>"+"Kill : "+kills;
            playing = false;
